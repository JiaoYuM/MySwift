...
class ViewController: UIViewController ,UITableViewDelegate,UITableViewDataSource{
    ...
    public let notificationName = NSNotification.Name.init("receivedIdSpotlightSearch")
    ...
    override func viewDidLoad() {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(receiveNotification(sender:)), name: notificationName, object: nil)
    ...
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        return self.dataSourcetu.count
    }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let itemDic = dataSourcetu[indexPath.row]
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        let itemDic = self.dataSourcetu[indexPath.row]
        let detailVC = DetailViewController.init(with: itemDic)
        self.navigationController?.pushViewController(detailVC, animated: true)
        
    }
    
    @objc func receiveNotification(sender:Notification) {
        let userInfo = sender.userInfo
        let index = Int(userInfo?["id"] as! String)
        let itemDic = dataSourcetu[index!]
        let detailVC = DetailViewController.init(with: itemDic)
        self.navigationController?.pushViewController(detailVC, animated: true)
    }
